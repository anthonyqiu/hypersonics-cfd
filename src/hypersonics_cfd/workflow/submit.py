...
POSTPROCESS_STEPS = ("yplus", "mirror", "slices", "shock")
POSTPROCESS_STEP_TIME_KEYS = {
    "yplus": "yplus_job_time",
...
The workflow can submit:
  solver -> yplus -> mirror -> slices -> shock
...
def selected_postprocess_steps(
    run_yplus: bool,
    run_mirror: bool,
    run_slices: bool,
    run_shock: bool,
...
def workflow_steps_from_args(
    args: argparse.Namespace,
) -> tuple[bool, bool, bool, bool, bool, bool, str] | None:
    explicit_step = (
        args.full_workflow or args.solver or args.yplus or args.mirror or args.slices or args.shock
    )
    ...
    if args.full_workflow or (noninteractive_cases and not explicit_step):
        return True, True, True, True, True, True, args.flow_file
    if explicit_step:
        run_postprocess = args.yplus or args.mirror or args.slices or args.shock
        return (
            args.solver,
            run_postprocess,
            args.yplus,
...
def choose_workflow_steps() -> tuple[bool, bool, bool, bool, bool, bool, str]:
    print("\nWorkflow defaults:")
    print("  solver -> extract surface y+ -> mirror half field -> export slices -> extract shock")
    print(f"  postprocess flow file: {FULL_FLOW_FILENAME}")

    if prompt_yes_no("Use the full workflow?", default=True):
        return True, True, True, True, True, True, FULL_FLOW_FILENAME
    ...
    if not run_postprocess:
        return run_solver, False, False, False, False, False, FULL_FLOW_FILENAME

    run_yplus = prompt_yes_no("Extract Orion surface y+?", default=True)
...
def postprocess_flags_for_step(
    ...
    if step == "yplus":
        return True, False, overwrite_outputs, False, False
    if step == "mirror":
        return False, True, overwrite_outputs, False, False
    if step == "slices":
        return False, False, overwrite_outputs, True, False
    if step == "shock":
        return False, False, overwrite_outputs, False, True
...
def build_postprocess_step_command(
    ...
    *,
    ...
    run_yplus, run_mirror, overwrite_outputs_flag, run_slices, run_shock = postprocess_flags_for_step(
        step,
        overwrite_outputs,
    )
...
def postprocess_step_inputs_ready(
    ...
    *,
    ...
    if step == "yplus":
        if complete_file(case_dir / RESTART_FILENAME) or complete_file(case_dir / "surface_flow.vtu"):
            return True
        print(f"{case_name}: skip yplus, missing {RESTART_FILENAME} and surface_flow.vtu")
        return False
...
def main() -> int:
    ...
    if not run_solver and not run_postprocess:
        ...
        return 0
    if run_postprocess and not run_yplus and not run_mirror and not run_slices and not run_shock:
    ...
    post_steps = selected_postprocess_steps(run_yplus, run_mirror, run_slices, run_shock)
    ...
    if run_postprocess:
        print(
            f"  Postprocess: yplus={run_yplus}, mirror={run_mirror}, slices={run_slices}, "
            f"shock={run_shock}, flow_file={flow_file}"
        )
    ...
    yplus_script = paths.repo_root / "scripts" / "extract_yplus_surface.py"
    ...
    if run_postprocess:
        ...
        if run_yplus and not yplus_script.exists():
            raise SystemExit(f"Missing workflow file: {yplus_script}")
