...
declare -a RESIDUAL_FILES=(
    "history.csv"
    "residual_history.svg"
)

declare -a YPLUS_FILES=(
    "orion_yplus.vtp"
    "yplus_summary.csv"
)
...
print_file_menu() {
    ...
    echo -e "  ${YELLOW}12)${RESET} residual history (.csv + .svg)"
    echo -e "  ${YELLOW}13)${RESET} Orion surface y+ files"
    echo ""
}
...
set_files_to_pull() {
    FILES_TO_PULL=()
    case "$1" in
        1|history) FILES_TO_PULL=("history.csv") ;;
        2|flow) FILES_TO_PULL=("flow_full.vtu") ;;
        3|shock) FILES_TO_PULL=("${SURFACE_FILES[@]}") ;;
        4|history-shock) FILES_TO_PULL=("history.csv" "${SURFACE_FILES[@]}") ;;
        5|history-flow) FILES_TO_PULL=("history.csv" "flow_full.vtu") ;;
        6|all) FILES_TO_PULL=("${PRIMARY_FILES[@]}") ;;
        7|light) FILES_TO_PULL=("${LIGHTWEIGHT_FILES[@]}") ;;
        8|diagnostics) FILES_TO_PULL=("${SEARCH_LINE_DIAGNOSTIC_FILES[@]}") ;;
        9|slices) FILES_TO_PULL=("${FLOW_SLICE_FILES[@]}") ;;
        10|terminated) FILES_TO_PULL=("${TERMINATED_SEARCH_LINE_FILES[@]}") ;;
        11|timing) FILES_TO_PULL=("${TIMING_FILES[@]}") ;;
        12|residuals) FILES_TO_PULL=("${RESIDUAL_FILES[@]}") ;;
        13|yplus) FILES_TO_PULL=("${YPLUS_FILES[@]}") ;;
        *) IFS=',' read -r -a FILES_TO_PULL <<< "$1" ;;
    esac
}
...
if [ "$#" -gt 0 ]; then
    set_files_to_pull "${2:-light}"
    direct_cases="${1//,/ }"
    read -r -a CASES <<< "${direct_cases}"
    pull_files "${CASES[@]}"
    exit 0
fi

print_file_menu
read -r -p "File type [1-13]: " file_choice
echo ""
